...
from state_simplify import state_simplify_ite, state_simplify_bv_width1, state_simplify_xvar
...
class SymbolicTraverse(object):
  ...
  def traverse_one_step(self, assumptions, branching_point: Sequence[TraverseBranchingNode], s_init = []):
    ...
    for idx in range(len(self.tracemgr.abs_state_one_step)):
      ...
      # if(len(self.tracemgr.abs_state_one_step)!= 1):
      # if (self.tracemgr.abs_eq(s_abs=self.tracemgr.abstract(abs_state), s2=state_init) == False):
      #   print('new state, need to simplify')
      state_simplify_xvar(abs_state_one_step, self.executor.get_Xs())
      sygus_simplify(abs_state_one_step, self.executor.get_Xs())
      # else:
      #   print('not new state, no need to simplify!')
      # abs_state_one_step.print()
      # # abs_state_one_step.print_assumptions()
      # #   input()
      # for s,v in abs_state_one_step.sv.items():
      #   if('RTL_if_id' in str(v.serialize())):
      #     print('idx:', idx)
      #     print('state:', s)
      #     assert False
    ...
    #   # xvar = self.executor.get_Xs()
    #   # fun = partial(simplify_process, xvar)
    #   # fun(abs_state_one_step)

    #   # simplify_process(xvar, abs_state_one_step)
    #   abs_state_one_step.print() 
    #   print('\n')

    # xvar = copy.copy(self.executor.get_Xs())
    # xvar_list = []
    # for idx in range(len(self.tracemgr.abs_state_one_step)):
    #   xvar_list.append(xvar)
    # assert len(xvar_list) == len(self.tracemgr.abs_state_one_step)
    # zip_args = list(zip(self.tracemgr.abs_state_one_step, xvar_list))
    # pool = multiprocessing.Pool(2)
    # pool.starmap(simplify_process, zip_args)
    # pool.close()
    # pool.join()

    # for state in self.tracemgr.abs_state_one_step:
    #   state.print()
  ...
  def traverse(self, assumptions, branching_point: Sequence[TraverseBranchingNode], s_init = []):
    ...
    for idx in range(len(self.tracemgr.abs_state)):
      ...
      # if(len(self.tracemgr.abs_state)!= 1):
      # if (self.tracemgr.abs_eq(s_abs=self.tracemgr.abstract(abs_state), s2=state_init) == False):
      #   print('new state, need to simplify')
      state_simplify_xvar(abs_state, self.executor.get_Xs())
      sygus_simplify(abs_state, self.executor.get_Xs())
    ...
    # for idx in range(len(self.tracemgr.abs_state)):
    #   xvar_list.append(xvar)
    # assert len(xvar_list) == len(self.tracemgr.abs_state)
    # zip_args = list(zip(self.tracemgr.abs_state, xvar_list))
    # pool = multiprocessing.Pool(2)
    # pool.starmap(simplify_process, zip_args)
    # pool.close()
    # pool.join()

    # for state in self.tracemgr.abs_state:
    #   state.print()
