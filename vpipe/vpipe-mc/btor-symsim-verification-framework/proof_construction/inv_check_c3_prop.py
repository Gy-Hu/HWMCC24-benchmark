...
def ila_trans():
    ...
    sts, _ = btor_parser.parse_file(Path("/data/wenjifang/SymSim-Framework/design/testcase4-four_stage_pipe2/problem_set.btor2"))
...
def main(extracted_list, sv_extracted_list):
    file_name = "/data/wenjifang/SymSim-Framework/output/branch_list_c3_set.pkl"
    ...
    asmpt_aux = And(Implies(tag_start, aux_and), Implies(tag_id, aux_and), Implies(tag_ex, aux_and), Implies(tag_wb, aux_and),Implies(tag_finish, aux_and_one))
    ...
    assert len(inv_group7.state_list) == len(finish_or_list)
    inv_finish_ila_rtl= Implies(tag_finish, \
                        Or( \
                            [And(inv_group7.state_list[idx],finish_or_list[idx]) \
    ...
    inv_ila_rtl0 = And(inv_start, inv_id, inv_ex, inv_wb, inv_finish_ila_rtl, ila_start, ila_id, ila_ex, ila_wb)

    init_check_generic_func(inv_ila_rtl0, sts_init_extended, asmpt)
    (check_result, cex) = inv_check_generic_func(inv_ila_rtl0, trans_update, asmpt, sts)
    property_check_generic_func(inv_ila_rtl0, sts_assertion, asmpt)
    ...
    while(check_result == False):
        ...
        if(n_tag0 == 1):
            ...
            list_1 = inv_group5.check_unsat_expr(list_1, cex_expr)
            list_2 = test_ce(inv_list6, cex)
            list_2 = inv_group6.check_unsat_expr(list_2, cex_expr)
            truelist = list_1 + list_2            
            tag_record_list.append('ppl_stage_wb')
        elif(n_tag4 == 1):
            print('ppl_stage_finish:',n_tag4)
            truelist = test_ce(inv_list7, cex)
            truelist = inv_group7.check_unsat_expr(truelist, cex_expr)            
        ...
        inv_ex = Or(inv_l3, inv_l4)
        inv_wb = Or(inv_l5, inv_l6)
        inv_ila_rtl = And(inv_start, inv_id, inv_ex, inv_wb, inv_finish_ila_rtl, ila_start, ila_id, ila_ex, ila_wb)
