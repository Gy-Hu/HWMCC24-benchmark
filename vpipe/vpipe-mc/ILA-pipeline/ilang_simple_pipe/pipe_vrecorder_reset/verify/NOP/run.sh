...
pono --vcd cex.vcd -e ic3ia  -k 50 -v 1  problem.btor2 
RfResult=$?
...
pono --vcd cex.vcd -e ic3ia  -k 50 -v 1  sanity.btor2 
SanityResult=$?
CoverResult=0
echo "* Parsing cover check input #0 ..."
yosys -s gen_cover_prop0.ys > __yosys_exec_result.cover0.txt
echo "Run Pono to check cover property (expecting sat)..."
pono -e bmc  -k 50 -v 1  cover0.btor2
if test $? != 0; then CoverResult=$?; fi
