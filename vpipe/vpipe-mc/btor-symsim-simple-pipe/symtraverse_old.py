...
from state_simplify import state_simplify_ite, state_simplify_bv_width1
...
class SymbolicTraverse(object):
  ...
  def traverse_one_step_filter_tag(self, assumptions, branching_point: Sequence[TraverseBranchingNode], tag_flag):
    ...
    while init_choice.has_valid_choice():
      ...
      is_new_state = self.tracemgr.record_state_w_asmpt(
        state,
        self.executor.get_Xs())  # record `state` in abs_state
      ...
      if is_new_state:
        for k,v in state.sv.items():
            if str(k)==tag_flag and str(v)=='0_1':
              ...
              expr_equal_0 = EqualsOrIff(expr, BV(0,1))
              expr_equal_1 = EqualsOrIff(expr, BV(1,1))
              sat_check_eq0 = And(expr_equal_0, assumpt_and)
              sat_check_eq1 = And(expr_equal_1, assumpt_and)
              sat_check_result_eq0 = is_sat(sat_check_eq0)
              sat_check_result_eq1 = is_sat(sat_check_eq1)
              # print('sat_check_result:',sat_check_result)
              assert not (sat_check_result_eq0 and sat_check_result_eq1)  # if it can possibly both be 1/0, what to do?
              if (sat_check_result_eq0 == False):
                # print('New tag state!')
                self.tracemgr.record_state_w_asmpt_one_step(state)  # record 'state' in abs_state_one_step
              else:
                pass
                # print ('Not new tag state!')
    ...
    for idx in range(len(self.tracemgr.abs_state_one_step)):
      ...
      state_simplify_ite(abs_state_one_step, self.executor.get_Xs())
  ...
  def traverse_one_step(self, assumptions, branching_point: Sequence[TraverseBranchingNode]):
    # the current state should be reachable /\ concrete enough /\ a new state
    state = self.executor.get_curr_state(assumptions)
    reachable = self.tracemgr.check_reachable(state)  # also include the assumption on current step
    assert reachable
  ...
  def traverse(self, assumptions, branching_point: Sequence[TraverseBranchingNode]):
    ...
    for idx in range(len(self.tracemgr.abs_state)):
      ...
      state_simplify_ite(abs_state, self.executor.get_Xs())
      # state_simplify_bv_width1(abs_state)
