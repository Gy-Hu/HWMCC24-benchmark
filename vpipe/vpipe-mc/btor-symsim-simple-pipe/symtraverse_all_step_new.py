...
def symtraverse_all_step():
  ...
  sts, _ = btor_parser.parse_file(Path("problem.btor2"))
  ...
  def extend_branch_next_phase(branch_list, executor, sts, base_sv, flag, phase_marker):
    ...
    for state_list_old in branch_list_old:
      ...
      traverser0_1.traverse_one_step(assumptions=tag2asmpt(flag=flag, executor_new=executor0_1), branching_point=order)
  ...
  def extend_branch_same_phase(branch_list, executor, sts, base_sv, flag, phase_marker):
    ...
    for state_list_old in branch_list_old:
      ...
      executor0_1.set_current_state(s, d)
      traverser0_1 = SymbolicTraverse(sts=sts, executor= executor0_1, base_variable=[executor0_1.sv(n) for n in base_sv])
      traverser0_1.traverse(assumptions=tag2asmpt(flag=flag, executor_new=executor0_1), branching_point=order)
  ...
  ##Step: start-ex
  print('Step: start-ex')
  extend_branch_next_phase(branch_list, executor, sts, base_sv, flag='start-ex', phase_marker=\
    {'__START__': 1, 'ppl_stage_ex': 0, 'ppl_stage_wb': 0, 'ppl_stage_finish': 0})

  ##Step: ex-ex
  print('\n\n\nStep: ex-->ex')
  extend_branch_same_phase(branch_list, executor, sts, base_sv=base_sv, flag='ex-ex', phase_marker=\
    {'__START__': 0, 'ppl_stage_ex': 1, 'ppl_stage_wb': 0, 'ppl_stage_finish': 0})

  ##Step: ex-wb
  print('\n\n\nStep: ex-->wb')
  extend_branch_next_phase(branch_list, executor, sts, base_sv={'RTL_ex_wb_val','RTL_ex_wb_rd','RTL_ex_wb_reg_wen','RTL_ex_wb_valid',\
  ...
  ##Step: wb-wb
  print('\n\n\nStep: wb-->wb')
  extend_branch_same_phase(branch_list, executor, sts, base_sv={'RTL_ex_wb_val','RTL_ex_wb_rd','RTL_ex_wb_reg_wen','RTL_ex_wb_valid',\
  ...
  ##Step: wb-finish
  print('\n\n\nStep: wb-->finish')
  extend_branch_next_phase(branch_list, executor, sts, base_sv= {'RTL_registers[0]','RTL_registers[1]','RTL_registers[2]','RTL_registers[3]'}, \
    flag='wb-finish', phase_marker={'__START__': 0, 'ppl_stage_ex': 0, 'ppl_stage_wb': 1, 'ppl_stage_finish': 0})

  file_name = "branch_list_with_rst2.pkl"
